...
plot_total_refugees <- ggplot(total_refugee_trends, aes(x = year, y = total_refugees)) +
  ...
  theme(axis.text = element_text(color = "black"),  # Ensure axis text is black for better contrast
...
plot2 <- ggplot(returned_refugee_trends, aes(x = year, y = returned_refugees)) +
  ...
  geom_smooth(method = "lm", formula = y ~ poly(x, 2), color = "#FF5230", se = FALSE) +
  labs(title = "Returned Refugee Populations Over Time", x = "Year", y = "Number of Returned Refugees (in K)") +
  theme_minimal(base_size = 14) +  # Increase base font size
  theme(axis.text = element_text(color = "black"),  # Ensure axis text is black for better contrast
        plot.title = element_text(size = 16, face = "bold"),  # Make title larger and bold
        axis.title = element_text(size = 14)) +  # Increase axis title size
  scale_x_continuous(breaks = seq(from = min(returned_refugee_trends$year), to = 2024, by = 1)) +
  scale_y_continuous(labels = scales::comma)
...
knitr::include_graphics("images/refugee_map_2012.jpg")
...
knitr::include_graphics("images/refugee_map_2018.jpg")
