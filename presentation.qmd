...
plot_total_refugees <- ggplot(total_refugee_trends, aes(x = year, 
                                                        y = total_refugees)) +
  geom_bar(stat = "identity", fill = "skyblue", 
           alpha = 0.7) +  # Bar plot for yearly trends
...
plot2 <- ggplot(returned_refugee_trends, aes(x = year, 
                                             y = returned_refugees)) +
...
plot_top_countries <- ggplot(top_country_total_refugees, 
  ...
  theme(axis.text = element_text(color = "black"),  # Ensure axis text is black for better contrast
        plot.title = element_text(size = 16, 
                                  face = "bold"),  # Make title larger and bold
...
plot_top_asylum_countries <- ggplot(top_country_total_asylum_seekers, 
  ...
  geom_bar(stat = "identity", 
           fill = "blue") +  # Bar plot showing total asylum seekers for each country
  ...
  updated_refugee_plot <- refugee_map_plot +
    theme(
      ...
      legend.key.size = unit(1.5, 
                             "lines"), # Adjusted for clarity in a map context
