...
author: "Datatude Dynamos- Kaarthik Sai, Kamaljeet Singh,  Surya Vardhan Dama, Ayesha Khatun, Mrunali Yadav, Rajitha "
title-slide-attributes:
  data-background-image: images/r04.jpg
...
## How have the patterns in refugee populations evolved over time, and how have the stances of American political parties impacted these developments? Analyse the dynamics of refugee migrations towards US political environments.{.smaller}
...
## Analysis: Refugee populations over time {.smaller auto-animate="true"}
...
## Analysis: Refugees by year and country of origin {.smaller auto-animate="true"}
...
plot_top_asylum_countries <- ggplot(top_country_total_asylum_seekers, aes(x = reorder(coa_name, total_asylum_seekers), y = total_asylum_seekers)) +
  ...
  coord_flip() +  # Flip coordinates to make it easier to read country names
  scale_y_continuous(labels = scales::comma) +
  theme(aspect.ratio = 6 / 12) # Format y-axis labels with commas for readability
...
## Analysis: Refugee populations over time with US political context. {.smaller auto-animate="true"}
...
-   A small number of refugees have returned over a period of time
    ...
    # Read the GIF
    gif <- image_read("images/anim_plot1.gif")

    # Resize the GIF to a specific width, maintaining aspect ratio
    # For example, resize to a width of 800 pixels
    gif_resized <- image_resize(gif, "500x")

    # Save the resized GIF
    image_write(gif_resized, "images/anim_plot1_resized.gif")
    ```

    ```{r, echo=FALSE}

    knitr::include_graphics("images/anim_plot1_resized.gif")
    ```

# Question 02 

## How does the global refugee population's migration to the USA vary across different countries and years? Are there external factors, such as COVID-19, war, climate change, financial stability, or policy changes, that influence the number of refugees seeking asylum in the USA? {.smaller}

## Data Preparation and Pre-processing

-   The processRefugees function refines the dataset by selecting key columns, appending missing countries with NA for uniformity, standardizing country names for consistency, categorizing refugee numbers for analytically clarity, and converting these categories to factors for effective visualization in R.
  ...
  filtered_data <- dataset |>
    ...
    mutate(
      coo_name = case_when(
        ...
        # Additional country name standardizations...
        TRUE ~ coo_name
      )
    ) |>
    mutate(
      refugees_m = case_when(
        ...
        # Additional refugee count categorizations...
        is.na(refugees) ~ "NA"
      )
    ) %>%
    mutate(
      refugees_m = factor(refugees_m, levels = c("<100", "100 to 500", "500 to 1000", "1k to 2k", "2k to 3k", "3k to 4k", "4k to 5k", "5k to 7k", "7k to 10k", "10k to 20k", "20k to 50k", "50k to 100k", "100k+", "NA"))
    )
...
```{r world_data, message=FALSE, include=FALSE, echo=FALSE}
library(maps)
library(ggplot2)
library(tibble)
library(dplyr)
...
```{r process_function, message=FALSE, echo=FALSE}
#| code-fold: true
...
```{r split_forest_data, message=FALSE, include=FALSE , echo=FALSE}
...
```{r colourMapping, message=FALSE, include=FALSE, echo=FALSE}
color_mapping <- c(
  "<100"           = "#d73027",  # Red
...
```{r world_plot_function, message=FALSE, echo=FALSE}
#| code-fold: true
...
-   External factors such as geopolitical conflicts, policy changes in the USA, economic instability, and global crises like the COVID-19 pandemic significantly influence refugee migration patterns. Among these, **geopolitical conflicts and policy changes** in the USA often have the most immediate and noticeable impact on the number of refugees seeking asylum.
...
-   Refugee populations from 2011 to 2022
...
knitr::include_graphics("images/th01.gif")
